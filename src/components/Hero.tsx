      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div ref={leftRef} className={`space-y-6 ${leftVisible ? 'opacity-100' : 'opacity-0'}`}>
            <div className={`inline-block bg-cyan-300 border-4 border-black px-4 py-2 rotate-2 ${leftVisible ? 'animate-fade-in' : ''}`}>
              <p className="font-bold text-sm tracking-wider">SALESFORCE DEVELOPER</p>
            </div>

            <h1 className={`text-6xl sm:text-7xl lg:text-8xl font-black leading-none ${leftVisible ? 'animate-slide-up animation-delay-200' : ''}`}>
            ...
            <p className={`text-xl sm:text-2xl font-bold bg-white border-4 border-black p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] ${leftVisible ? 'animate-slide-up animation-delay-400' : ''}`}>
              Building <span className="bg-yellow-300 px-2">SCALABLE</span> enterprise{' '}
              <span className="bg-rose-400 px-2">SOLUTIONS</span>
            </p>

            <div className={`flex flex-wrap gap-4 ${leftVisible ? 'animate-fade-in animation-delay-600' : ''}`}>
          ...
          <div ref={rightRef} className={`space-y-4 ${rightVisible ? 'opacity-100' : 'opacity-0'}`}>
            <div className={`bg-orange-400 border-4 border-black p-6 rotate-2 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] ${rightVisible ? 'animate-slide-left' : ''}`}>
            ...
            <div className={`bg-yellow-300 border-4 border-black p-6 -rotate-1 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] ${rightVisible ? 'animate-slide-left animation-delay-200' : ''}`}>
            ...
            <div className={`bg-cyan-300 border-4 border-black p-6 rotate-1 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] ${rightVisible ? 'animate-slide-left animation-delay-400' : ''}`}>
